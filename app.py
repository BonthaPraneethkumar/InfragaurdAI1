...
app = FastAPI(title="InfraGuard AI", version="2.0.1")
    ...
    if PRIVACY_STRICT:
        try:
            ...
            for c in sorted(contours, key=cv2.contourArea, reverse=True)[:80]:
                ...
                if roi.size == 0:
                    continue
                # Analyze document texture on a bounded-size copy to avoid large temporary
                # label arrays on free/small Render instances.
                roi_scan = roi
                if max(roi.shape[:2]) > 640:
                    rs = 640 / max(roi.shape[:2])
                    roi_scan = cv2.resize(roi, (max(1, int(roi.shape[1]*rs)), max(1, int(roi.shape[0]*rs))), interpolation=cv2.INTER_AREA)
                ...
                bright_fraction = float(np.mean(roi_scan > 150))
                if bright_fraction < 0.45:
                    continue
                th = cv2.threshold(roi_scan, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)[1]
                n, labels, stats, _ = cv2.connectedComponentsWithStats(th, connectivity=8)
                small_components = 0
                scan_h, scan_w = roi_scan.shape[:2]
                for st in stats[1:]:
                    cw, ch, ca = int(st[cv2.CC_STAT_WIDTH]), int(st[cv2.CC_STAT_HEIGHT]), int(st[cv2.CC_STAT_AREA])
                    if 2 <= cw <= max(35, scan_w//5) and 3 <= ch <= max(30, scan_h//6) and 5 <= ca <= max(800, int(scan_w*scan_h*0.02)):
                        small_components += 1
...
def privacy_redact(data: bytes) -> tuple[np.ndarray, dict[str, Any]]:
    ...
    This hotfix decodes camera photos through Pillow and downsizes them *before* the
    OpenCV privacy pipeline. That keeps memory predictable on small cloud instances
    while preserving enough detail for privacy detection and vision analysis.
    """
    try:
        with Image.open(BytesIO(data)) as pil:
            # Respect phone-camera EXIF rotation before any detection. JPEG draft/thumbnail
            # lets Pillow avoid expanding a multi-megapixel photo at full working size.
            try:
                pil.draft("RGB", (1280, 1280))
            except Exception:
                pass
            pil = ImageOps.exif_transpose(pil).convert("RGB")
            pil.thumbnail((1280, 1280), Image.Resampling.LANCZOS, reducing_gap=2.0)
            rgb = np.asarray(pil)
            if rgb.size == 0:
