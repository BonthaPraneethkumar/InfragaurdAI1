...
$("#startPermissionsBtn").addEventListener("click", async ()=>{
  try{
    // Browser/OS permission prompts are mandatory the first time. After permission is granted,
    // location can attach automatically to every captured photo.
...
async function processPhoto(file){
  ...
  try{
    ...
    $("#redactionBadge").textContent=n?`Privacy protected · ${n} area${n===1?"":"s"} hidden`:"Privacy check complete";
...
async function startVoiceAutomatically(){
  try{
    let audioTracks=state.stream?.getAudioTracks().filter(t=>t.readyState==="live")||[];
    if(!audioTracks.length){
      const mic=await navigator.mediaDevices.getUserMedia({audio:true});
      if(state.stream)mic.getAudioTracks().forEach(t=>state.stream.addTrack(t)); else state.stream=mic;
      audioTracks=state.stream.getAudioTracks();
    }
    ...
    state.recorder.onstop=async()=>{
      ...
      $("#processingText").textContent="Sarvam Saaras automatically detects the Indian language and translates the speech to English text.";
      await transcribeAndAnalyze();
    };
    state.voiceSeconds=20;
    $("#voiceTimer").textContent="Speak naturally in any supported Indian language · 00:20";
    state.recorder.start();
    state.voiceTimer=setInterval(()=>{
      state.voiceSeconds--;
      $("#voiceTimer").textContent=`Speak naturally in any supported Indian language · 00:${String(Math.max(0,state.voiceSeconds)).padStart(2,"0")}`;
      if(state.voiceSeconds<=0&&state.recorder.state==="recording")state.recorder.stop();
    },1000);
  }catch(e){
    $("#voiceTitle").textContent="Microphone unavailable";
    $("#voiceTimer").textContent="You can continue by typing your complaint.";
    setTimeout(()=>showManualAnalysis(""),900);
  }
}
...
async function transcribeAndAnalyze(){
  try{
    ...
    showManualAnalysis("");
    toast("Sarvam voice service is not configured. Type the English complaint manually.");
  }
}
...
async function analyzeText(text,sourceNote=""){
  $("#processingTitle").textContent="Understanding the complaint…";
  $("#processingText").textContent="AI is determining issue type, severity and responsible department.";
  try{
    const r=await api("/api/analyze",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text,location_label:state.locationLabel})});
    state.analysis=r.analysis;
    $("#processingPanel").classList.add("hidden");$("#analysisStage").classList.remove("hidden");
    $("#englishTranscript").textContent=text;$("#editText").value=text;
    renderAnalysis(r.analysis,r.live_ai?sourceNote:"Local fallback · add Sarvam key for live AI");
...
$("#submitBtn").addEventListener("click",async()=>{
  ...
  try{
    const r=await api("/api/submit",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({
      description:$("#editText").value.trim(),transcript:state.transcript,location_label:state.locationLabel,
