...
async function analyzeText(text,sourceNote=""){
  ...
  try{
    ...
    renderAnalysis(r.analysis,(r.live_ai?sourceNote:"Fallback analysis")+photoSource,r.photo_analysis||null);
  }catch(e){showManualAnalysis(text);toast(e.message)}
}
function renderAnalysis(a,source,photo=null){
  ...
  const conf=photo?.confidence_percent ?? (photo?.confidence!=null?photo.confidence*100:null);
  $("#aiPhotoConfidence").textContent=conf!=null?`${Number(conf).toFixed(1)}%`:"Unavailable";
  const area=photo?.affected_area_percent;
  $("#aiAffectedArea").textContent=area!=null&&Number(area)>0?`~${Number(area).toFixed(1)}% of image`:"Not measurable";
  const obs=photo?.observations||"";
  const note=photo?.measurement_note||"";
  $("#aiPhotoObservation").textContent=[obs,note].filter(Boolean).join(" · ");
