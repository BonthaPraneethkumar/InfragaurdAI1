...
async function processPhoto(file){
  ...
  try{
    ...
    // V4.3: verify the PHOTO ITSELF before requesting microphone permission.
    // Voice/text must never be used to make an unrelated photo look valid.
    $("#processingTitle").textContent="Verifying infrastructure issue in photo…";
    $("#processingText").textContent="InfraGuard is checking the privacy-protected photo before requesting microphone access.";
    const verify=await api("/api/verify-photo",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({
      evidence_token:state.evidenceToken
    })});

    if(!verify.photo_gate?.accepted){
      state.analysis=null;
      state.transcript="";
      ...
      $("#englishTranscript").textContent="Microphone was not requested because the photo did not pass infrastructure verification.";
      $("#editText").value="";
      $("#aiIssue").textContent=verify.photo_analysis?.available?"Photo rejected":"Photo verification unavailable";
    ...
    $("#voiceTitle").textContent="Photo verified ✓ · Allow microphone, then speak";
    $("#voiceTimer").textContent=`Photo verified ${Math.round((Number(verify.photo_gate.confidence)||0)*100)}% · Your browser will ask for microphone permission.`;
