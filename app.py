...
async def sarvam_photo_analyze(evidence_path: Path) -> dict[str, Any]:
    """Strict photo gate using Sarvam Gemma 4 on the privacy-protected image only.

    This function never uses the citizen voice/text to decide whether the image itself
    contains a visible civic infrastructure problem. If vision is unavailable, callers
    should fail closed instead of accepting a complaint from text alone.
    """
    if not (sarvam_ready() and SARVAM_VISION_ENABLED):
        return {
            ...
            "issue_type": "Other",
            "severity": "Medium",
            "observations": "Photo verification is unavailable because vision is not enabled.",
            "confidence": 0.0,
    ...
    try:
        ...
        prompt = (
            "You are InfraGuard AI's STRICT civic-infrastructure photo verifier. Inspect ONLY this "
            "privacy-protected image. Do not use any voice, text complaint, filename, location, or assumptions. "
            "Decide whether a clearly visible PUBLIC INFRASTRUCTURE problem is actually present. "
            "Accepted visible classes are: Traffic obstruction/illegal parking, Garbage Waste, broken Streetlight, "
            "Road Pothole/road damage, Water Leaks, exposed/dangerous Electric Wires, or Cut down/fallen trees on road. "
            "Reject normal roads with no defect, ordinary buildings/rooms, selfies/people, food, vehicles with no obstruction, "
            "random objects, scenic photos, unclear/blurry photos, or any image where the issue is not visibly verifiable. "
            "Do NOT invent an issue. If uncertain, set is_infrastructure_issue=false. "
            "Return ONLY one JSON object with exactly these fields: "
            "is_infrastructure_issue (boolean), issue_type, severity, observations, confidence. "
            "issue_type must be exactly one of: Traffic, Garbage Waste, Streetlight, Road Pothole, Water Leaks, "
            "Electric Wires, Cut down trees on road, Other. severity must be Low, Medium, High, or Critical. "
            "confidence must be a number from 0 to 1 representing confidence that the visible issue classification is correct."
        )
        payload = {
        ...
        async with httpx.AsyncClient(timeout=65) as client:
            ...
            if r.status_code >= 400:
                detail = r.text[:300]
                raise RuntimeError(f"Gemma 4 photo verification failed ({r.status_code}): {detail}")
        ...
        if sev not in {"Low", "Medium", "High", "Critical"}:
            sev = "Medium"
        raw_bool = parsed.get("is_infrastructure_issue", False)
        if isinstance(raw_bool, bool):
            is_issue = raw_bool
        else:
            is_issue = str(raw_bool).strip().lower() in {"true", "1", "yes"}
        try:
            conf = max(0.0, min(1.0, float(parsed.get("confidence", 0.0))))
        except Exception:
            conf = 0.0
        # "Other" cannot pass the strict gate because the visible problem was not classified
        # into one of InfraGuard's supported civic issue types.
        if issue == "Other":
            is_issue = False
        return {
            "available": True,
            "is_infrastructure_issue": is_issue,
            "issue_type": issue,
            "severity": sev,
        ...
        # Safe diagnostic for Render logs. Never print the API key or request headers.
        print(f"[InfraGuard vision] {type(exc).__name__}: {str(exc)[:500]}", flush=True)
        return {
            "available": False,
            "is_infrastructure_issue": False,
            "issue_type": "Other",
            "severity": "Medium",
            "observations": "Live AI could not verify the photo. Please retry with a clear infrastructure-issue photo.",
            "confidence": 0.0,
...
class PhotoVerifyBody(BaseModel):
    evidence_token: str
...
async def transcribe(file: UploadFile = File(...)):
    ...
    filename = file.filename or "citizen-report.wav"
    ...
    # IMPORTANT: InfraGuard needs meaning translation, not transliteration.
    # Saaras v3 + mode=translate is the supported speech -> English path.
    primary_error = None
    try:
        ...
        detected_audio_language = result.get("language_code") or "unknown"

        # Safety net: if the STT result is still recognized as an Indian language
        # (for example Romanized Telugu), translate that text to English once more.
        try:
            async with httpx.AsyncClient(timeout=30) as client:
                lid = await client.post(
                    "https://api.sarvam.ai/text-lid",
                    headers={**headers, "Content-Type": "application/json"},
                    json={"input": english[:1000]},
                )
            if lid.status_code < 400:
                lid_json = lid.json()
                text_language = lid_json.get("language_code")
                if text_language and text_language != "en-IN":
                    ...
                    if tr.status_code < 400:
                        translated = (tr.json().get("translated_text") or "").strip()
                        if translated:
                            english = translated
        except Exception:
            # Do not fail a valid Saaras translation just because the safety check failed.
            pass
    ...
    # Last-resort translation endpoint. This still returns English meaning, never transliteration.
    try:
        async with httpx.AsyncClient(timeout=70) as client:
            r = await client.post(
                "https://api.sarvam.ai/speech-to-text-translate",
                headers=headers,
                data={"model": "saaras:v2.5", "language_code": "unknown"},
                files={"file": (filename, raw, content_type)},
            )
            if r.status_code >= 400:
                raise ValueError(f"Sarvam translate fallback {r.status_code}: {sarvam_error(r)}")
            result = r.json()

        english = (result.get("transcript") or "").strip()
        if not english:
            raise ValueError("Sarvam translation fallback returned an empty transcript.")

        return {
            "english_text": english,
            "detected_language": result.get("language_code"),
            "language_probability": result.get("language_probability"),
            "model": "saaras:v2.5-translate-fallback",
            "warning": f"Primary v3 translate path failed: {primary_error}"[:280],
        }
    except Exception as exc:
...
async def verify_photo(body: PhotoVerifyBody):
    """Verify the privacy-protected photo BEFORE microphone capture.

    This endpoint intentionally ignores voice/text. If live vision is unavailable or the
    image does not show a supported infrastructure issue with >=75% confidence, it fails
    closed and the browser must not request microphone permission.
    """
    if not body.evidence_token:
        return {
            "photo_gate": {
                "accepted": False,
                "reason": "No photo was supplied. Take a clear photo of the infrastructure problem.",
                "confidence": 0.0,
            },
            "photo_analysis": {"available": False, "source": "no-photo"},
        }
    ...
    if not evidence.exists():
        return {
            "photo_gate": {
                "accepted": False,
                "reason": "The photo has not passed privacy processing. Please take the photo again.",
                "confidence": 0.0,
            },
            "photo_analysis": {"available": False, "source": "missing-evidence"},
        }
    ...
    if not photo.get("available"):
        return {
            "photo_gate": {
                "accepted": False,
                "reason": photo.get("observations") or "Photo verification is temporarily unavailable. Please retry.",
                "confidence": 0.0,
                "technical_error": photo.get("error", ""),
            },
            "photo_analysis": photo,
        }

    confidence = float(photo.get("confidence", 0.0) or 0.0)
    if (not photo.get("is_infrastructure_issue")) or confidence < min_confidence:
        reason = photo.get("observations") or "No clear infrastructure issue is visible in this photo."
        return {
            "photo_gate": {
                "accepted": False,
                "reason": f"No clear infrastructure issue detected. {reason}"[:500],
    ...
    return {
        "photo_gate": {
            "accepted": True,
            "reason": photo.get("observations", "Visible infrastructure issue verified."),
            "confidence": confidence,
        },
        "photo_analysis": photo,
    }
...
async def analyze(body: AnalyzeBody):
    ...
    # STRICT PHOTO GATE: a complaint cannot proceed unless the privacy-protected photo
    # is independently verified by the vision model as a visible supported infrastructure issue.
    if not body.evidence_token:
        return {
            "photo_gate": {
                "accepted": False,
    ...
    # The image itself passed. Only now may voice/text contribute to routing and severity.
    if sarvam_ready():
        try:
    ...
    # Downstream text fallback is allowed only because the photo already passed the strict gate.
    if text:
        result = fallback_analysis(text)
        # Prefer the verified photo category when the text classifier cannot classify it.
        if result.get("issue_type") == "Other":
            issue = normalize_issue(str(photo.get("issue_type", "Other")))
            result.update({
                "issue_type": issue,
                "severity": str(photo.get("severity", "Medium")),
                "department": DEPT[issue],
                "reason": "Verified photo issue used because live combined routing was unavailable.",
                "confidence": confidence,
                "ai_source": str(photo.get("source", "verified-photo")),
            })
    else:
        issue = normalize_issue(str(photo.get("issue_type", "Other")))
        result = {
            ...
            "severity": str(photo.get("severity", "Medium")),
            "department": DEPT[issue],
            "summary": str(photo.get("observations", "Visible infrastructure issue verified."))[:220],
            "reason": "Photo-only routing used after strict visual verification.",
            "confidence": confidence,
            "ai_source": str(photo.get("source", "verified-photo")),
        }
    return {
